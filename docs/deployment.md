...
curl -fL -o kixdns-panel.zip \
  https://github.com/tuoro/kixdns-panel/releases/download/v3.2.0/kixdns-panel-linux-x86_64.zip
...
- **Releases**：跟随上游正式发布（`v0.1.0` 早于增强协议，无法提供）

版本按上游先后排列：Actions 按上游运行的先后，Releases 按版本号。只有上游更新的版本，或同一版本的更高补丁集，才会提示更新；同一个版本重新打包不改变顺序，也不提示。

依赖出现安全公告时，当前版本会自动换上修补后的依赖重新构建，面板提示“依赖安全升级”，本地版本的补丁集后面带上修订号，例如 `p9-r1`。修补不了的旧版本不再续建，到期后从列表消失，已下载到本机的不受影响。
