...
fn notifies_only_for_newer_panel_release_with_matching_asset() {
    ...
    let same = release("v1.0.24", panel_release_asset_name());
    assert!(
        !to_panel_update_notice(None, Some("v1.0.24"), Some(&same))
            .unwrap()
            .available
    );
    assert!(
    ...
    let wrong_asset = release("v1.0.24", "kixdns-panel-windows.zip");
    assert!(
    ...
    let newer = release("v1.0.24", panel_release_asset_name());
    let notice = to_panel_update_notice(Some(TEST_BUILD_COMMIT), None, Some(&newer)).unwrap();
    assert!(notice.available);
    assert_eq!(notice.current_version, "1.0.24");
    assert_eq!(notice.latest_version.as_deref(), Some("1.0.24"));
    assert_eq!(
        notice.download_url.as_deref(),
        Some(
            format!(
                "https://github.com/tuoro/kixdns-panel/releases/download/v1.0.24/{}",
