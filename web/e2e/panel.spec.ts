...
test('域名映射方案可在一张卡片中维护多条 CNAME', async ({ page }) => {
  await open(page, '/config')
  await page.getByRole('button', { name: '一键添加方案', exact: true }).click()

  const guide = page.getByRole('dialog', { name: '一键添加 DNS 方案' })
  await guide.getByRole('button', { name: /域名映射/ }).click()
  await guide.getByLabel('映射 1 源域名').fill('alias.example')
  await guide.getByLabel('映射 1 目标域名').fill('origin.example.')
  await guide.getByLabel('映射 1 TTL').fill('120')
  await guide.getByRole('button', { name: '添加映射' }).click()
  await guide.getByLabel('映射 2 源域名').fill('alias-two.example')
  await guide.getByLabel('映射 2 目标域名').fill('origin-two.example.')
  await expect(guide.locator('.solution-guide__preview').first()).toContainText('2 条域名映射')
  await guide.getByRole('button', { name: '创建方案', exact: true }).click()

  const created = page.locator('.solution-card').filter({ hasText: 'alias.example' })
  await expect(created).toHaveCount(1)
  await expect(created).toContainText('2 条 CNAME 映射')
  await expect(created).toContainText('alias-two.example')
  await created.getByRole('button', { name: '一键编辑' }).click()
  const editGuide = page.getByRole('dialog', { name: '一键编辑 DNS 方案' })
  await expect(editGuide.getByLabel('映射 2 目标域名')).toHaveValue('origin-two.example.')
  await editGuide.getByRole('button', { name: '关闭一键方案' }).click()
