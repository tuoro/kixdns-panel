...
Action 是 Release 的先行版：新增强先上 Action，Release 只在上游发布新版本时跟进，跟进时用 Action 当时的补丁集，编号与 Action 相同。所以 Release 的编号永远不超过 Action；上游没发新版本期间 Action 有了新增强，Release 的号会暂时比 Action 小。Action 的编号只在两种情况下变大：增强内容改变，或者通用补丁因为上游改了挂钩点附近的代码不得不改。上游只是依赖锁或入口写法变了，一律新增兼容层，编号不变。版本本身的新旧由上游身份（Run 编号、Release 标签）表达，面板的排序和更新提示也只按上游身份比较。

封印有两个例外，都以整个兼容层目录为单位：

- 可以新增一个**名字未用过**的兼容层。已有的锁不会选中新名字，已有构建因此不变；另一份上游借此沿用同一个编号。自动生成的兼容层按上游身份命名：Action 用 `run-<运行编号>`，Release 用标签。
- 可以整个删除一个**没有锁在用**的兼容层。仍有锁在用时锁校验会报缺失，清理脚本在旧版本移出版本目录后删除这些兼容层。

往已有兼容层里增删改文件、新增 `release/<tag>/`（按标签自动选中）以及改动其余任何文件，都会改变已有构建，仍然只能新增更高编号。
...
同步任务按 Action、Release 两条轨道独立验证，两条轨道都先保编号：

- **Action**，上游有新的成功构建时：先原样套用当前补丁集；不行就**并入**，即 `cargo xtask rebase --join`，给当前补丁集新增一个兼容层，编号不变；并入失败说明通用补丁也要改，才执行下面的**完整重基**领新编号；重基冲突时开 `[compat]` Issue。
- **Release**，只在上游发布新版本时：先原样套用 Action 当前的补丁集和兼容层；不行就并入 Action 的补丁集，即 `cargo xtask rebase --join --base-source action`；并入失败就开 `[compat]` Issue，Release 轨道停在当前版本等人工处理。Release 从不退回自己的旧补丁集，也从不从旧补丁集重基出新编号，因此不会带着旧增强发布，编号也不会超过 Action。

并入与完整重基共用前三步：从版本目录找到基准提交及其补丁集，把兼容层、Release 专用层和通用层依次重建为临时 Git 提交链（排除所有 `Cargo.lock` 差异，避免依赖锁文件的行号和版本变化制造无意义冲突），rebase 到候选上游，再用固定 Rust 工具链重新解析依赖。重新解析的依赖锁只对这一份上游成立：基准有兼容层时放进兼容层，通用层才能原样给别的上游共用。之后两者分开：

- 并入只导出兼容层，在候选上游的干净检出上先应用它、再原样应用补丁集其余各层；任何一个补丁不能原样应用就放弃，报“并入失败”，不写入任何东西。成功后兼容层加进基准补丁集，候选锁指向它。
- 完整重基导出整个补丁集：已经被上游吸收的空提交会被丢弃，Release 专用补丁映射到新标签；复制能力清单、导出更高编号的暂存补丁集，通过密封校验后才同时启用新补丁集和候选锁。

无论并入还是完整重基，之后都对新源码树运行格式、测试（库与 `kixdns` 二进制）、Clippy、RustSec、构建和 DNS 冒烟；全部成功后才提交 PR 并自动合并，PR 正文写明是并入还是重基。

整个生成过程使用暂存目录；失败时不会写入半个补丁集或切换当前锁。基础设施或工具错误只会让 Action 失败，不创建兼容性告警。只有 Git rebase 报告代码冲突、依赖无法解析、Release 并入失败或完整验证失败时，工作流才创建或更新对应的 `[compat]` Issue；失败类型只看最后一次尝试，Action 并入失败后转入完整重基是预料之中的，不算告警；同一轨道只保留一个开放告警。失败候选不会进入 `upstreams/`，另一条轨道和已发布 Artifact 不受影响。
...
3. 仅有版本结构差异、增强内容不变时，优先在 Action 当前的补丁集里新增一个名字未用过的 `compatibility/<名称>/`（Release 用标签命名），给候选锁设置 `patchset` 和 `compatibility`，编号不变；Release 也并入 Action 当前的补丁集，不回到自己的旧补丁集。需要改动已有补丁时才创建新的补丁集编号。
